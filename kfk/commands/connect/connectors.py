...
CONNECTOR_SKIPPED_PROPERTIES = (
    ...
    SpecialTexts.CONNECTOR_TASKS_MAX,
    SpecialTexts.CONNECTOR_CLASS,
)
...
@click.option(
    "--describe",
    "is_describe",
    help="List details for the given connector.",
    is_flag=True,
)
@click.argument(
    "config_file",
    type=click.File("r"),
    cls=argument_extensions.NotRequiredIf,
...
@click.option(
    "--connector",
    help="Connector Name",
    cls=option_extensions.RequiredIf,
...
def create(config_file, cluster, namespace):
    with open(
        ...
        connector_dict["metadata"]["name"] = connector_properties.get(
            SpecialTexts.CONNECTOR_NAME
        ).data
        connector_dict["metadata"]["labels"]["strimzi.io/cluster"] = cluster

        connector_dict["spec"]["class"] = connector_properties.get(
            SpecialTexts.CONNECTOR_CLASS
        ).data
        connector_dict["spec"]["tasksMax"] = int(
            connector_properties.get(SpecialTexts.CONNECTOR_TASKS_MAX).data
        )
...
def alter(config_file, cluster, namespace):
    ...
    add_properties_config_to_resource(
        connector_properties, connector_dict["spec"]["config"], _return_if_not_skipped
    )
