...
@click.option(
    "--config",
    help="A topic configuration override for the topic being created or altered.",
    multiple=True,
)
@click.option(
    "--alter",
    "is_alter",
    help=(
        "Alter the number of partitions, replica assignment, and/or configuration of"
        " the topic."
    ),
    is_flag=True,
)
...
@click.option(
    "--command-config",
    help=(
        "Property file containing configs to be config property file passed to Admin"
        " Client."
    ),
)
@click.option(
    "--native",
    help="List details for the given topic natively.",
    is_flag=True,
    cls=RequiredIf,
    options=["is_describe"],
)
...
@click.option(
    "--describe", "is_describe", help="List details for the given topic.", is_flag=True
)
@click.option(
    "--replication-factor",
...
@click.option(
    "--partitions",
    help="The number of partitions for the topic being created or altered.",
    cls=RequiredIf,
    options=["is_create"],
    type=int,
)
...
@click.option(
    "--topic", help="Topic Name", required=True, cls=NotRequiredIf, options=["is_list"]
)
@kfk.command()
def topics(
    topic,
...
def alter(
    topic, partitions, replication_factor, config, delete_config, cluster, namespace
):
    stream = get_resource_as_stream(
        "kafkatopics", resource_name=topic, namespace=namespace
    )
