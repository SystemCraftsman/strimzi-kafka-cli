...
oc extract secret/my-user -n kafka --keys=user.crt --to=- > user.crt
oc extract secret/my-user -n kafka  --keys=user.key --to=- > user.key
oc extract secret/my-cluster-cluster-ca-cert -n kafka --keys=ca.crt --to=- > ca.crt
