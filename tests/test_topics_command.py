...
class TestKfkTopics(TestCase):
    ...
    def test_describe_topic_native(self, mock_os):
        ...
        assert result.exit_code == 0
        native_command = (
            "bin/kafka-topics.sh --bootstrap-server {cluster}-kafka-brokers:9092 --describe --topic {"
            "topic}"
        )
        mock_os.system.assert_called_with(
            Kubectl()
    ...
    def test_describe_topic_native_with_command_config(
        ...
        assert result.exit_code == 0
        native_command = (
            "bin/kafka-topics.sh --bootstrap-server {cluster}-kafka-brokers:9093 --describe --topic {"
