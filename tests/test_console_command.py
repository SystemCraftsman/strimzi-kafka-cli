...
class TestKfkConsole(TestCase):
    ...
    def test_console_consumer(self, mock_os):
        ...
        native_command = (
            "bin/kafka-console-consumer.sh --bootstrap-server my-cluster-kafka-brokers:9092 --topic {"
            "topic} "
        )
    ...
    def test_console_consumer_with_consumer_config(
        ...
        native_command = (
            "bin/kafka-console-consumer.sh --bootstrap-server {cluster}-kafka-brokers:9093 --topic {"
    ...
    def test_console_producer_with_producer_config(
        ...
        assert result.exit_code == 0
        native_command = (
            "bin/kafka-console-producer.sh --broker-list {cluster}-kafka-brokers:9093 --topic {topic} "
            "--producer.config /tmp/client.properties;"
            "rm -rf /tmp/truststore.jks;rm -rf /tmp/user.p12;rm -rf /tmp/client.properties;"
        )
