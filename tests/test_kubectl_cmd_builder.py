...
class TestKubectl(TestCase):
    ...
    def test_exec(self):
        self.assertEqual(
            Kubectl().exec("-it", "test-pod").container("test-container").namespace("test-namespace").exec_command(
                "echo 'test'").build(),
            "{kubectl} exec -it test-pod -c test-container -n test-namespace -- bash -c \"echo 'test'\"".format(
    ...
    def test_kafkausers_specific(self):
        self.assertEqual(Kubectl().get().kafkausers("my-user").build(),
                         "{kubectl} get kafkausers my-user".format(kubectl=KUBECTL_PATH))

    def test_configmap_yaml(self):
        self.assertEqual(Kubectl().get().configmap("my-cluster-kafka-config").output("yaml").build(),
                         "{kubectl} get configmap my-cluster-kafka-config -o yaml".format(kubectl=KUBECTL_PATH))

    def test_label(self):
        self.assertEqual(Kubectl().get().kafkatopics().label("app=test").build(),
    ...
    def test_namespace(self):
        self.assertEqual(Kubectl().get().kafkatopics().namespace("test").build(),
                         "{kubectl} get kafkatopics -n test".format(kubectl=KUBECTL_PATH))

    def test_namespace_all(self):
        self.assertEqual(Kubectl().get().kafkatopics().namespace().build(),
