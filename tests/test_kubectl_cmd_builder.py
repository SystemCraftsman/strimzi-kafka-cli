...
class TestKubectl(TestCase):
    def test_get(self):
        self.assertEqual(Kubectl().get().build(), "{kubectl} get".format(kubectl=KUBECTL_PATH))

    def test_create(self):
        self.assertEqual(Kubectl().create().build(), "{kubectl} create".format(kubectl=KUBECTL_PATH))

    def test_apply(self):
        self.assertEqual(Kubectl().apply().build(), "{kubectl} apply".format(kubectl=KUBECTL_PATH))

    def test_describe(self):
        self.assertEqual(Kubectl().describe().build(), "{kubectl} describe".format(kubectl=KUBECTL_PATH))

    def test_delete(self):
        self.assertEqual(Kubectl().delete().build(), "{kubectl} delete".format(kubectl=KUBECTL_PATH))
    ...
    def test_kafkas_specific(self):
        self.assertEqual(Kubectl().get().kafkas("my-cluster").build(), "{kubectl} get kafkas my-cluster".format(kubectl=KUBECTL_PATH))

    def test_kafkatopics_all(self):
        self.assertEqual(Kubectl().get().kafkatopics().build(), "{kubectl} get kafkatopics".format(kubectl=KUBECTL_PATH))

    def test_kafkatopics_specific(self):
        self.assertEqual(Kubectl().get().kafkatopics("my-topic").build(), "{kubectl} get kafkatopics my-topic".format(kubectl=KUBECTL_PATH))

    def test_kafkausers_all(self):
        self.assertEqual(Kubectl().get().kafkausers().build(), "{kubectl} get kafkausers".format(kubectl=KUBECTL_PATH))

    def test_kafkausers_specific(self):
        self.assertEqual(Kubectl().get().kafkausers("my-user").build(), "{kubectl} get kafkausers my-user".format(kubectl=KUBECTL_PATH))

    def test_label(self):
        self.assertEqual(Kubectl().get().kafkatopics().label("app=test").build(), "{kubectl} get kafkatopics -l app=test".format(kubectl=KUBECTL_PATH))

    def test_labels(self):
        self.assertEqual(Kubectl().get().kafkatopics().label("app=test, included=true").build(),
                         "{kubectl} get kafkatopics -l app=test, included=true".format(kubectl=KUBECTL_PATH))
    ...
    def test_output(self):
        self.assertEqual(Kubectl().get().kafkas().output("yaml").build(), "{kubectl} get kafkas -o yaml".format(kubectl=KUBECTL_PATH))
