...
class TestKfkConnect(TestCase):
    ...
    def test_no_option(self):
        result = self.runner.invoke(connect, ['clusters', '--cluster', self.cluster, '-n', self.namespace])
    ...
    def test_create_cluster_three_replicas(self, mock_os, mock_click_confirm, mock_open_file_in_system_editor,
        ...
        result = self.runner.invoke(connect,
                                    ['clusters', '--create', '--cluster', self.cluster, '--replicas', 3,
    ...
    def test_list_clusters(self, mock_os):
        result = self.runner.invoke(connect, ['clusters', '--list', '-n', self.namespace])
        assert result.exit_code == 0
    ...
    def test_delete_cluster(self, mock_os, mock_click_confirm):
        mock_click_confirm.return_value = True
        result = self.runner.invoke(connect, ['clusters', '--delete', '--cluster', self.cluster, '-n', self.namespace])
        assert result.exit_code == 0
        mock_os.system.assert_called_with(
    ...
    def test_alter_cluster_without_parameters(self, mock_os):
        result = self.runner.invoke(connect, ['clusters', '--alter', '--cluster', self.cluster, '-n', self.namespace])
        assert result.exit_code == 0
        mock_os.system.assert_called_with(
    ...
    def test_alter_cluster_with_replica_param(self, mock_os, mock_get_resource_yaml, mock_create_temp_file):
        with open(r'files/yaml/kafka-connect.yaml') as file:
            ...
            result = self.runner.invoke(connect,
                                        ['clusters', '--alter', '--cluster', self.cluster, '--replicas', 3, '-n',
                                         self.namespace])
            assert result.exit_code == 0
