...
from src.kfk.utils import snake_to_camel_case
...
@click.option(
    "--delete-quota",
    "delete_quota_tuple",
    help="User quotas to be removed.",
    multiple=True,
)
@click.option(
    "--quota",
    "quota_tuple",
    help="User's network and CPU utilization quotas in the Kafka cluster.",
    multiple=True,
)
@click.option(
    "--resource-pattern-type",
    help=(
        "The type of the resource pattern or <ANY|MATCH|LITERAL|PREFIXED> pattern"
        " filter. When adding acls, this should be a specific pattern type, e.g."
...
@click.option(
    "--resource-name",
    help="ACL resource name.",
    cls=RequiredIf,
    options=["add_acl", "delete_acl"],
)
@click.option(
    "--resource-type",
    help="ACL resource type.",
    type=click.Choice(["topic", "group", "cluster"], case_sensitive=True),
    cls=RequiredIf,
    options=["add_acl", "delete_acl"],
)
@click.option(
    "--type",
    help="Operation type for ACL. (default: allow)",
    type=click.Choice(["allow", "deny"], case_sensitive=True),
    default="allow",
)
@click.option(
    "--host", help="Host which User will have access. (default: *)", default="*"
)
@click.option(
    "--operation",
    "operation_tuple",
...
@click.option(
    "--authorization-type",
    help="Authorization type for user",
    type=click.Choice(["none", "simple"], case_sensitive=True),
)
@click.option(
    "--alter",
    "is_alter",
    help="Alter authentication-type, quotas, etc. of the user.",
    is_flag=True,
)
...
@click.option(
    "--describe", "is_describe", help="List details for the given user.", is_flag=True
)
@click.option(
    "--authentication-type",
    help="Authentication type for user",
    type=click.Choice(["tls", "scram-sha-512"], case_sensitive=True),
    cls=RequiredIf,
    options=["is_create"],
)
...
@click.option(
    "--user", help="User Name", required=True, cls=NotRequiredIf, options=["is_list"]
)
@kfk.command()
def users(
    user,
...
def alter(
    ...
    stream = get_resource_as_stream(
        "kafkausers", resource_name=user, namespace=namespace
    )
    ...
    if len(quota_tuple) > 0:
        if user_dict["spec"].get("quotas") is None:
            user_dict["spec"]["quotas"] = {}
        add_kv_config_to_resource(
            quota_tuple, user_dict["spec"]["quotas"], snake_to_camel_case
        )

    if len(delete_quota_tuple) > 0:
        if user_dict["spec"].get("quotas") is not None:
            delete_resource_config(
                delete_quota_tuple, user_dict["spec"]["quotas"], snake_to_camel_case
            )
